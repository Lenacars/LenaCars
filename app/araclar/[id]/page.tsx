...
import Link from "next/link"; // Link importu önemli
import {
  ...
  // Kullanmadığınız ChevronLeft, ChevronRight, Tag, Gauge, Heart, ShoppingCart, BookOpen gibi ikonları silebilirsiniz.
} from "lucide-react";
...
interface Vehicle {
  ...
  kisi_kapasitesi?: string; // Ekledim, eğer yoksa Vehicle interface'inize ekleyin
}
...
export default function Page({ params }: Props) {
  ...
  const allSpecs = [
    ...
    { label: "Stok Kodu", value: vehicle.stok_kodu, iconName: "HelpCircle"}, // Stok kodu için varsayılan ikon
  ...
  return (
    <div className="bg-gray-50 min-h-screen">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-8">
        ...
        <div className="bg-white shadow-2xl rounded-xl overflow-hidden">
            ...
            <div className="lg:col-span-3">
                <div className="relative w-full aspect-[16/10] bg-gray-100 group overflow-hidden"> {/* aspect-16/10 daha geniş bir görünüm için */}
                {selectedImage ? (
                    <Image 
                        src={selectedImage} 
                        alt={vehicle.isim || "Araç Kapak Fotoğrafı"} 
                        layout="fill" 
                        objectFit="cover" // contain yerine cover daha dolgun gösterebilir, test edin
            ...
            <div className="lg:col-span-2 p-6 sm:p-8 flex flex-col bg-white">
                <h1 className="text-2xl lg:text-3xl font-extrabold text-gray-900 leading-tight mb-1">{vehicle.isim}</h1>
                ...
                <div className="mt-auto space-y-3 pt-4">
                    ...
                    <button 
                        ...
                        disabled={isVehicleAddingToGarage || isVehicleAddedToGarage} // Garajdaysa da tekrar eklenemesin
                        className={`w-full flex items-center justify-center px-6 py-3 border rounded-lg shadow-sm text-base font-medium transition-all duration-150 ease-in-out transform hover:scale-[1.02] focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-[#6A3C96] ${
                            isVehicleAddedToGarage 
                                ? "bg-green-500 text-white border-green-500 cursor-not-allowed" // Daha belirgin "eklendi" stili
      ...
      {/* Detaylar, Açıklama, Yorumlar için Ortak Kart */}
      <div className="mt-8 sm:mt-10 bg-white shadow-xl rounded-lg p-6 sm:p-8">
        {/* TODO: Buraya sekmeli yapı (Tabs) eklenebilir. Şimdilik alt alta. */}
            ...
            <div className="mt-10 pt-6 border-t border-gray-300">
                ...
                <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mt-3">
                        ...
                        <div className="flex space-x-1">
                            {[5, 4, 3, 2, 1].map((r) => (
                                <button 
                                    ...
                                    title={`${r} Yıldız`}
                                    className={`p-2 rounded-full transition-all duration-150 ease-in-out transform hover:scale-110 ${newRating === r ? 'bg-[#6A3C96] text-yellow-400 shadow-md' : 'bg-gray-200 text-gray-500 hover:bg-gray-300'}`}
                                >
                    ...
                    <button 
                        ...
                        disabled={!newComment.trim() || newComment.length < 10} // Min yorum uzunluğu örneği
      ...
      <div className="md:hidden sticky bottom-0 left-0 right-0 bg-white p-3 border-t border-gray-200 shadow-[0_-2px_10px_rgba(0,0,0,0.1)] z-50"> {/* Daha belirgin üst gölge */}
        <div className="flex items-center justify-between gap-3">
            <div className="flex-shrink-0 text-left">
                <div className="text-[#6A3C96] text-xl font-bold leading-tight"> {/* Font büyüdü */}
            ...
            <button 
                onClick={handleRentNow}
                className="w-auto flex-grow flex items-center justify-center px-4 py-3 border border-transparent rounded-lg shadow-sm text-sm font-semibold text-white bg-[#6A3C96] hover:bg-[#58307d] focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-[#6A3C96] transition-colors" // Padding ve font-semibold eklendi
            >
                <CreditCard size={18} className="mr-2"/> {/* İkon boyutu sabit */}
                Hemen Kirala
            </button>
        </div>
      </div>
