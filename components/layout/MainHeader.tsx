...
export default function MainHeader() {
  ...
  const searchContainerRef = useRef<HTMLDivElement>(null); // Masaüstü arama için
  const mobileSearchContainerRef = useRef<HTMLDivElement>(null); // Mobil arama önerileri için (opsiyonel)
  ...
  useEffect(() => {
    if (searchTerm.trim().length > 1) {
      const desktopRef = searchContainerRef.current;
      const mobileRef = mobileSearchContainerRef.current; // Bu ref mobil arama formunun kendisine atanmalı
      
      // Hangi arama kutusunun aktif olduğuna bağlı olarak önerileri filtrele
      // Şimdilik tek bir 'suggestions' state'i olduğu için bu ayrım karmaşıklaşabilir.
      // Basitlik adına, öneriler her iki durumda da aynı şekilde filtrelenecek.
  ...
  useEffect(() => {
    function handleClickOutside(event: MouseEvent) {
      // Masaüstü arama önerilerini kapat
      if (searchContainerRef.current && !searchContainerRef.current.contains(event.target as Node)) {
        // Sadece masaüstü arama önerilerini kapatmak için bir yol bulmamız gerekebilir,
        // eğer mobil ve masaüstü önerileri ayrı yönetiliyorsa.
        // Şimdilik genel önerileri kapatıyor.
        // setSuggestions([]);
      }
  ...
  const handleSearchFormSubmit = (e: FormEvent<HTMLFormElement>) => {
    ...
    setSuggestions([]); // Arama yapıldığında önerileri temizle
  ...
  const handleSuggestionClick = (suggestion: VehicleSuggestion) => {
    ...
    setSuggestions([]); // Öneriye tıklanınca önerileri temizle
    const vehicleListElement = document.getElementById('vehicle-list');
    if (vehicleListElement) {
      vehicleListElement.scrollIntoView({ behavior: "smooth" });
    }
  ...
  return (
    <header className="sticky top-0 z-50 bg-white">
      {/* Üst Bilgi Çubuğu - Önceki önerideki gibi */}
        ...
        <div className="container mx-auto flex items-center justify-center md:justify-between">
          <div className="flex items-center space-x-4 md:space-x-3">
            ...
            <Link href="/iletisim" className="flex items-center hover:text-gray-200 transition-colors" aria-label="E-posta">
              <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5 mr-1" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z" /></svg>
            </Link>
          ...
          <div className="text-center hidden md:block flex-grow mx-4">
      ...
      {/* Ana Başlık Barı - Logo, Arama, Kullanıcı Butonları */}
      {/* === DEĞİŞİKLİK BAŞLANGICI: Mobil ve Masaüstü için ayrı düzen === */}
      <div className="bg-white py-3 px-4 shadow-sm border-b border-gray-100">
        <div className="container mx-auto">
          {/* Mobil Layout: Hamburger Sol - Logo Orta - Arama İkonu Sağ */}
          <div className="md:hidden flex items-center justify-between">
            ...
            <div className="flex-1 text-center"> {/* Orta: Logo */}
              <Link href="/" className="inline-block">
                <Image
                  src="/LENACARS.svg"
                  alt="LenaCars Logo"
                  width={160} // Mobil için biraz daha küçük logo
                  height={45}
                  className="h-10 w-auto" // Tailwind class'ı ile asıl boyut kontrolü
                  priority
          ...
          {/* Masaüstü Layout: Logo Sol - Arama Kutusu Orta - Kullanıcı Butonları Sağ */}
            ...
            <div> {/* Sol: Logo */}
              <Link href="/" className="flex-shrink-0">
                <Image
                  src="/LENACARS.svg"
                  alt="LenaCars Logo"
                  width={230} // Masaüstü orijinal boyut
                  height={64}
                  className="h-16 w-auto" // Tailwind class'ı ile asıl boyut kontrolü
                  priority
                  onError={(e) => { const target = e.target as HTMLImageElement; if (target.src.endsWith(".svg")) { target.src = "/LENACARS.png"; }}}
                />
              </Link>
            </div>
            {/* Orta: Arama Kutusu (Genişleyebilir) */}
            <div ref={searchContainerRef} className="flex-1 px-6"> {/* flex-1 ile genişlemesini sağla, px-6 ile yanlardan boşluk */}
              <form
                onSubmit={handleSearchFormSubmit}
                className="relative w-full max-w-xl mx-auto flex items-center group border border-gray-300 rounded-lg overflow-hidden shadow-sm hover:shadow-md focus-within:ring-2 focus-within:ring-[#6A3C96] focus-within:border-transparent transition-all duration-200"
              >
                ...
                <input
                  ...
                  onFocus={() => { if (searchTerm.trim().length > 1 && allVehiclesForSuggestions.length > 0 && !isMobile) { /* Önerileri göster */ } }}
                />
                <button
                  type="submit"
              ...
              {!isMobile && suggestions.length > 0 && (
                <ul className="absolute top-full left-0 right-0 mt-1.5 z-[60] bg-white border border-gray-200 rounded-lg shadow-xl max-h-[calc(100vh-200px)] overflow-y-auto py-1.5 w-full"> {/* w-full eklendi */}
      ...
      {isMobile && isMobileMenuOpen && (
        ...
        >
          <div className="fixed top-0 left-0 h-full w-[85%] max-w-xs bg-white shadow-xl z-50">
            <NavigationMenu
      ...
      {isMobile && isMobileSearchVisible && (
        <div ref={mobileSearchContainerRef} className="bg-gray-50 p-3 border-t border-b border-gray-200 relative"> {/* ref eklendi, relative eklendi */}
           <form onSubmit={handleSearchFormSubmit} className="relative flex items-center group border border-gray-300 rounded-lg overflow-hidden shadow-sm focus-within:ring-2 focus-within:ring-[#6A3C96] focus-within:border-transparent transition-all duration-200">
            ...
            {/* Mobil Arama Önerileri (Mobil arama kutusunun hemen altında) */}
            {isMobile && suggestions.length > 0 && (
              <ul className="absolute top-full left-3 right-3 mt-1.5 z-[60] bg-white border border-gray-200 rounded-lg shadow-xl max-h-[calc(100vh-200px)] overflow-y-auto py-1.5"> {/* Pozisyon ayarlandı */}
                  {suggestions.map((vehicle) => (
                    <li key={vehicle.id}>
                      <button
                        type="button"
